...
interface ResizePluginOption {
    ...
    toolbar?: {
        ...
        border?: boolean;       // Show border button (default: true)
        align?: boolean;        // Show alignment buttons (default: true)
        wrap?: boolean;         // Show text wrapping buttons (default: true)
        imageActions?: {        // Configure image action buttons
            copy?: boolean;       // Show copy button (default: true)
            link?: boolean;       // Show link button (default: true)
            altText?: boolean;    // Show alt text button (default: true)
...
class ResizePlugin {
    ...
    positionResizerToTarget(el: HTMLElement) {
        ...
        try {
            ...
            const leftHandle = this.resizer.querySelector('.left-side-bar') as HTMLElement;
            const rightHandle = this.resizer.querySelector('.right-side-bar') as HTMLElement;
            
            if (!leftHandle || !rightHandle) return;
            
            // First, determine the actual alignment type based on multiple properties
            let alignmentType = 'default';
            
            // Extract numeric values from margin strings for comparison
            const getMarginValue = (marginStr: string): number => {
                const match = marginStr.match(/([\d.]+)/);
                return match ? parseFloat(match[1]) : 0;
            };
            
            const leftMarginValue = getMarginValue(marginLeft);
            const rightMarginValue = getMarginValue(marginRight);
            
            // Log the extracted margin values
            console.log('Extracted margin values:', { leftMarginValue, rightMarginValue });
   ...
   */
    ...
    startResize(e: MouseEvent) {
        ...
        if (target.classList.contains("handler") && e.which === 1) {
            ...
            if (this.resizer) {
                const allHandlers = this.resizer.querySelectorAll('.handler');
                allHandlers.forEach(handler => {
                    (handler as HTMLElement).classList.add('active');
                });
            }
        }
    }
    endResize() {
        ...
        const handlers = this.resizer?.querySelectorAll('.handler');
        handlers?.forEach(handler => {
            (handler as HTMLElement).classList.remove('active');
        });
    ...
    resizing(e: MouseEvent) {
        ...
        const isLeftHandler = target.classList.contains('left-side-bar');
        const isRightHandler = target.classList.contains('right-side-bar');
    ...
    editAltText() {
        ...
        // Create modal
        const modal = this.createModal(altTextModalTemplate);
        document.body.appendChild(modal);

        // Get the current alt text
        const currentAltText = this.resizeTarget.getAttribute('alt') || '';

        // Set current alt text in the input field
        const altTextInput = modal.querySelector('#quill-resize-alt-text') as HTMLInputElement;
        if (altTextInput) {
            altTextInput.value = currentAltText;
            setTimeout(() => (altTextInput as any).focus(), 100);
            
            // Real-time update as user types
            altTextInput.addEventListener('input', () => {
                const newAltText = altTextInput.value.trim();
                this.resizeTarget.setAttribute('alt', newAltText);
        ...
        const backButton = modal.querySelector('.quill-resize-modal-backArrow');
        const closeButton = modal.querySelector('.quill-resize-modal-close');
        
        if (backButton) backButton.addEventListener('click', handleModalAction);
